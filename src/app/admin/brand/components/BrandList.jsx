...
export default function BrandList({ id }) {
    ...
    async function searchData() {
      ...
      try{
        const result = await axiosClientAPI.get(`brand?search=${search}`, config)
        .then((response) => {
          setData(response.data.data)
          setPrevURL(response.data.links.prev)
          setNextURL(response.data.links.next)
    ...
    async function deleteData(id) {
      try{
        const result = await axiosClientAPI.delete(`brand/${id}`, config)
        .then((response) => {
      ...
      {data.length > 0 ?
        data.map((item, i) => (
          <div className='w-[90%] border-x border-b border-slate-300 mx-auto flex items-center justify-start '>
            <div className='w-[50%] border-r border-blue-300 px-3 py-2'>{item.name}</div>
            ...
            <div className='w-[20%] px-3 py-2 flex items-center justify-start gap-4'>
              <Link href={`/admin/brand/${item.id}`}> <FaEye className='hover:text-blue-500 duration-150 hover:scale-110 transition-all ease-in'/> </Link>
              <Link href={`/admin/brand/edit/${item.id}`}> <MdEdit className='hover:text-green-500 duration-150 hover:scale-110 transition-all ease-in' /> </Link>  
              <button> 
                  <MdDeleteForever 
